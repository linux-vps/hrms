...
export class TaskService {
  ...
  async findByAssignee(employeeId: string): Promise<Task[]> {
    this.logger.log(`Đang tìm task cho nhân viên với ID: ${employeeId}`);
    ...
    try {
      ...
      this.logger.log(`Đã tìm thấy ${tasks.length} task cho nhân viên với ID: ${employeeId}`);
      
      // Nếu không tìm thấy task nào, kiểm tra xem nhân viên có tồn tại không
      if (tasks.length === 0) {
        ...
        if (!employee) {
          this.logger.warn(`Không tìm thấy nhân viên với ID: ${employeeId}`);
        } else {
          this.logger.log(`Nhân viên ${employee.fullName} (ID: ${employeeId}) không được giao task nào`);
        }
      }
      
      return tasks;
    } catch (error) {
      this.logger.error(`Lỗi khi tìm task cho nhân viên: ${error.message}`);
      throw error;
    }
  }
  ...
  async updateStatus(id: string, updateTaskStatusDto: UpdateTaskStatusDto, employeeId: string): Promise<Task> {
    ...
    // Chuyển từ WAITING_REVIEW sang COMPLETED hoặc REJECTED (chỉ supervisor)
      ...
      if (!isSupervisor && !isAssigner) {
        throw new ForbiddenException('Chỉ người giám sát hoặc người giao việc mới có thể xác nhận hoàn thành hoặc từ chối công việc');
      }
        ...
        for (const assignee of task.assignees) {
          await this.mailService.sendMail(
            ...
            'task-review-result',
            {
              ...
              taskTitle: task.title,
              taskId: task.id,
              supervisorName: employee.fullName || (isSupervisor ? 'Người giám sát' : 'Người giao việc'),
  ...
  /**
   * Lấy danh sách công việc của nhân viên theo dự án
   */
  async findTasksInEmployeeProjects(employeeId: string): Promise<Task[]> {
    this.logger.log(`Đang tìm task trong các dự án của nhân viên với ID: ${employeeId}`);
    
    try {
      // Tìm tất cả dự án mà nhân viên là thành viên
      const employeeProjects = await this.projectRepository
        .createQueryBuilder('project')
        .innerJoinAndSelect('project.members', 'member', 'member.id = :employeeId', { employeeId })
        .getMany();
      
      this.logger.log(`Nhân viên với ID ${employeeId} tham gia ${employeeProjects.length} dự án`);
      
      if (employeeProjects.length === 0) {
        return [];
      }
      
      // Lấy ID của tất cả dự án
      const projectIds = employeeProjects.map(project => project.id);
      
      // Tìm tất cả task trong các dự án đó
      const tasks = await this.taskRepository.find({
        where: { projectId: In(projectIds) },
        relations: ['project', 'assigner', 'supervisor', 'assignees', 'subtasks'],
      });
      
      this.logger.log(`Đã tìm thấy ${tasks.length} task trong các dự án của nhân viên với ID: ${employeeId}`);
      
      return tasks;
    } catch (error) {
      this.logger.error(`Lỗi khi tìm task trong dự án của nhân viên: ${error.message}`);
      throw error;
    }
  }
} 